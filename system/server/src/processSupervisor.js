...
export class SupervisedProcessGroup extends EventEmitter {
  ...
  start(key, bin, args) {
    ...
    const entry = { child: null, log: [], restartCount: 0, restartTimer: null, stopped: false };
  ...
  _spawnNow(key, bin, args) {
    ...
    const child = this.spawn(bin, args, { stdio: ["ignore", "pipe", "pipe"] });
    entry.child = child;
    ...
    child.on("exit", (code, signal) => {
      ...
      entry.restartTimer = setTimeout(() => this._spawnNow(key, bin, args), delay);
